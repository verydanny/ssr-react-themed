...
export default class LeadModal extends Component {
  ...
  constructor(props) {
    ...
    const cookieData = getLeadData(this.props.cookies)

    const optInNewsletter = this.props.isCountryUS ? 1 : 0
  ...
  get header() {
    const { theme, mPhone, isPaid, revenue } = this.props

    const phoneStr = this._phone || (this._phone = formatPhone(mPhone))
  ...
  get leadFields() {
    ...
    const allFields = [
      ...
      },
      {
        ...
        defaultValue: leadData.move_date,
  ...
  submitLead(event, formData) {
    ...
    const mergedData = {
      ...
      opt_in_newsletter: optInNewsletter,
      move_date: isoToLocaleDate(formData.move_date),
      endecaId,
      listingId,
      tplSource,
  ...
  render() {
    ...
    return (
      <Fragment>
        ...
        />
        <div key="lead_modal" className={theme.LeadModal}>
          <LeadModalUI
